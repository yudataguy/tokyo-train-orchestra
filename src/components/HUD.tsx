...
export default function HUD({ lines, recentArrivals, weather, onSettingsClick }: HUDProps) {
  ...
  return (
    <>
      ...
      <div className="absolute top-3 left-3 sm:top-4 sm:left-4 z-[1000] bg-[var(--paper)]/85 backdrop-blur-sm border border-[var(--rule)] rounded-[2px] px-3 py-2" aria-label={t('tokyo')}>
        <div className="tnum text-2xl sm:text-3xl font-bold text-[var(--ink)]"><TokyoTime /></div>
        <div className="text-xs sm:text-sm text-[var(--ink-2)]">
          {t('tokyo')}
          {weather && (
            <>
              {' · '}
      ...
      <div className="absolute top-3 right-3 sm:top-4 sm:right-4 z-[1000] flex gap-2">
        <button type="button" onClick={onSettingsClick} className="w-10 h-10 bg-[var(--paper)]/85 backdrop-blur-sm border border-[var(--rule)] rounded-[2px] flex items-center justify-center text-[var(--ink-2)] hover:text-[var(--ink)] hover:border-[var(--ink)] cursor-pointer transition-colors" aria-label={t('settings')} aria-haspopup="dialog">
      ...
      <div className="absolute bottom-1 right-2 z-[1000] text-[10px] text-[var(--ink-2)] pointer-events-none">
      ...
      <section
        className="absolute bottom-0 left-0 right-0 z-[1000] bg-gradient-to-t from-[var(--paper)] via-[var(--paper)]/90 to-transparent px-4 pt-6 pb-2"
        aria-label={t('citySleeps')}
      >
        {displayedArrivals.length === 0 ? (
          <div className="text-[var(--ink-2)] text-sm">{t('citySleeps')}</div>
        ) : (
          <ul
            ...
            {displayedArrivals.map((event) => {
              ...
              return (
                <li key={(event as any)._seq ?? `${event.trainId}-${event.timestamp}`} className="flex items-center gap-2 whitespace-nowrap">
                  <span className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ backgroundColor: line.color }} aria-hidden="true" />
                  <span className="text-xs font-semibold text-[var(--ink)]">{lineName}</span>
                  <span className="text-xs text-[var(--ink-2)]">{tInstrument(line.instrument)} &middot; {stationName}</span>
                </li>
              );
            })}
          </ul>
