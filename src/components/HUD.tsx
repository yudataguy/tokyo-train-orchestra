            ...
            {displayedArrivals.map((event) => {
              ...
              return (
                <div key={`${event.trainId}-${event.timestamp}`} className="flex items-center gap-2 whitespace-nowrap">
                  ...
                  <span className="text-xs font-semibold" style={{ color: line.color }}>{line.name}</span>
                  <span className="text-xs text-gray-500">{line.instrument} &middot; {station?.name ?? event.station}</span>
                </div>
              );
            })}
          </div>
