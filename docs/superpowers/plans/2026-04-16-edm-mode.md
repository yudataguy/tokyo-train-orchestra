...
Also add `releaseAll?: () => void;` to the `PolySynth` mock in the test file (inside the returned mock factory object), and add `releaseAll: jest.fn()` to keep the mock shape honest.
